...
class Unit(System):
    ...
    def test_viability(self):
        ...
        if (self.thetas[-1] > self.upper_viability) or (self.thetas[-1] < self.lower_viability):
            return False # return False for not viable
        return True # return True for viable
    

    ### Experimental Test Viability Function ###

    # def test_viability(self):

    #     # test whether  the Unit is within viable limits
    #     if (np.sum(self.thetas[-10:-1]) > self.upper_viability) or (np.sum(self.thetas[-10:-1]) < self.lower_viability):
    #         return False # return False for not viable
    #     return True # return True for viable
...
class Homeostat(System):
    ...
    def __init__(self, n_units, upper_viability, lower_viability, adapt_fun, upper_limit=np.Inf, lower_limit=-np.Inf, weights_set=None, test_interval=10, adapt_enabled=True,adaptation_cooldown=50,mutation_rate=0.1, crossover_rate=0.3):
    ...
    def step(self, dt):
        ...
        for unit in self.units:
            unit.step(dt)
            if not unit.test_viability(): # collect the units that are not in the viable range
                not_viable_units.append(unit)

        if not_viable_units and self.units[0].adapt_enabled and self.time_since_adaptation >= self.adaptation_cooldown: # has enough time to time passed
            self.adjust_weights(dt, not_viable_units) # GA / PSO / SA (all optimisations happen)
    ...
    def adjust_weights(self, dt, not_viable_units):
        #### TEST CASES AND GA / PSO ####

        # unit_instances = self.get_unit_instances()

        # all_new_weights = [random.uniform(-1, 1) for _ in range(len(not_viable_units) * 4)]
        # all_new_weights = linear_ga(n_genes=len(not_viable_units) * len(self.units),homeostat_cp=self.deep_copy(),mutation_rate=self.mutation_rate,crossover_rate=self.crossover_rate)
        all_new_weights = linear_pso(n_genes=len(not_viable_units) * len(self.units),homeostat_cp=self.deep_copy(),c1=self.mutation_rate,c2=self.crossover_rate)


        # 4 units total; 3 units distributed -> 12 new weights
        ...
        for i, unit in enumerate(not_viable_units):
            unit_new_weights = all_new_weights[i * weights_per_unit:(i + 1) * weights_per_unit] # breaks down the list of weights into the right size pieces
